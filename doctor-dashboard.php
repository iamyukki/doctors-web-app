<?php 
	...
	if(isset($_GET['accept'])){
		$sql = "UPDATE appointments SET status = 'confirm' WHERE idAppointment = ".$_GET['accept'];
		mysqli_query($conn,$sql);
		header('location:my-patients.php');
	}
	if(isset($_GET['cancel'])){
		$sql = "UPDATE appointments SET status = 'cancelled' WHERE idAppointment = ".$_GET['cancel'];
		mysqli_query($conn,$sql);
	}
	$sql = "SELECT *,count(appointments.idClient) as clients,count(idAppointment) as aps FROM appointments WHERE idDoctor = ".$_SESSION['id']." AND status = 'pending'";
	...
	$appointsRow = mysqli_fetch_assoc($res);
?>
												...
												<div class="col-md-12 col-lg-4">
													<div class="dash-widget dct-border-rht">
														...
														<div class="dash-widget-info">
															<h6>Today Patient</h6>
															<h3>1</h3>
															<p class="text-muted"><?= (new DateTime())->format('d M Y') ?></p>
														</div>
													</div>
												</div>
								...
								<div class="col-md-12">
									...
									<div class="appointment-tab">
										...
										<ul class="nav nav-tabs nav-tabs-solid nav-tabs-rounded">
											...
											<li class="nav-item">
												<a class="nav-link" href="#" data-toggle="tab">Today</a>
											</li> 
										</ul>
										<!-- /Appointment Tab -->
															...
															<table class="table table-hover table-center mb-0">
																...
																<tbody>
																	<?php
																		...
																		$sql = "SELECT * FROM checkouts WHERE idClient = ".$appointsRow['idClient'];
																		$checkout = mysqli_fetch_assoc(mysqli_query($conn,$sql));
																		
																		$sql = "SELECT * FROM clients WHERE idClient = ".$appointsRow['idClient'];
																	...
																	<tr>
																		<td>
																			<h2 class="table-avatar">
																				<a href="patient-profile.html" class="avatar avatar-sm mr-2"><img class="avatar-img rounded-circle" src="assets/img/patients/<?= $client['profile_pic'] ?>" alt="User Image"></a>
																				<a href="patient-profile.html"><?= $client['first_name'] .' '. $client['last_name'] ?> <span>#PT<?= $client['idClient'] ?></span></a>
																			</h2>
																		</td>
																		<td><?= (new DateTime($appointsRow['date']))->format('d M Y') ?><span class="d-block text-info">10.00 AM</span></td>
																		<td>General</td>
																		<td>New Patient</td>
																		<td class="text-center">$<?= $checkout['total'] ?></td>
																		<td class="text-right">
																			<div class="table-action">
																				<a href="invoice-view.php?id=<?= $appointsRow['idAppointment'].'&idCli='.$client['idClient'] ?>" class="btn btn-sm bg-info-light">
																					<i class="far fa-eye"></i> View
																				</a>
																				
																				<a href="?accept=<?= $appointsRow['idAppointment']?>" class="btn btn-sm bg-success-light">
																					<i class="fas fa-check"></i> Accept
																				</a>
																				<a href="?cancel=<?= $appointsRow['idAppointment']?>" class="btn btn-sm bg-danger-light">
